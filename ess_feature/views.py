...
from .serializers import (
    ...
    ProtectionMapFeatureTestSerializer,
    ProtectionMapFeatureTestLogLevelCountSerializer,
)
...
class ProtectionMapFeatureTestView(ListAPIView):
    serializer_class = ProtectionMapFeatureTestSerializer
    filter_backends = [DjangoFilterBackend]
    filterset_fields = ["error_code", "level"]
    ...
    def get_queryset(self):
        ...
        if time_query_param is not None:
            queryset = ProtectionMapFeatureTest.objects.filter(
                operating_site=operating_site_id, timestamp=time_query_param
            ).order_by("-timestamp")
...
class ProtectionMapFeatureTestAllListView(ListAPIView):
    serializer_class = ProtectionMapFeatureTestSerializer
    ...
    def get_queryset(self):
        ...
        # If 'time' parameter is included, other parameters are ignored.
        if time:
            queryset = ProtectionMapFeatureTest.objects.filter(timestamp=time).order_by("-timestamp")
        else:
            queryset = ProtectionMapFeatureTest.objects.filter(
...
class ProtectionMapFeatureTestLogLevelCountView(ListAPIView):
    def get_queryset(self):
        return None
    ...
    def get(self, request, *args, **kwargs):
        try:
            ...
            with connections["ess_feature"].cursor() as cursor:
                query = """
                    SELECT time_bucket(%(time_bucket_width)s, "TIMESTAMP" AT TIME ZONE %(time_bucket_timezone)s) "time", "OPERATING_SITE" "operating_site", "LEVEL" "level", count("LEVEL") "level_count" 
                    FROM protectionmap_feature_new_test
                    WHERE "OPERATING_SITE" = %(operating_site_id)s AND "TIMESTAMP" BETWEEN %(start_time)s AND %(end_time)s 
                    GROUP BY "time", "operating_site", "level" 
                    ORDER BY "time" DESC, "level"
                """
            ...
            serializer = ProtectionMapFeatureTestLogLevelCountSerializer(data=data, many=True)
