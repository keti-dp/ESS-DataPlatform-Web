...
function getPrimaryMonitoringLogAlertElement(data) {
    ...
    let alertMessage = `Operating Site ${data['operating_site']} / Bank ${data['bank_id']} / Rack ${data['rack_id']} - ${data['error_code']['description_eng']}`;
    let alertLevel;
...
function getMonitoringLogAlertElement(data) {
    ...
    alertElement.setAttribute('title', data['description_eng']);
    alertElement.innerHTML = `
        <p class="text-truncate m-t-0 m-b-0">
            <small>Bank ${data['bank_id']} / Rack ${data['rack_id']}</small>
        </p>
        <p class="text-truncate m-t-0 m-b-0">
            <small>${data['description_eng']}</small>
        </p>
    `;
 ...
 */
 function createMonitoringLog(elementId, data, getAlertElementFunc) {
    ...
    if (Array.isArray(data) && data.length == 0) {
        let message = `${DateTime.now().toFormat(customFullDateTimeFormat)}<br><strong>It's normally working at today.</strong>`;
 ...
 */
async function primaryMonitoringLogLevelTypeCountSelectHandler(event, mode) {
    if (mode !== 'normal' && mode !== 'test') {
        console.log("'mode' parameter value must be 'normal' or 'test'");
    ...
    if (Array.isArray(data) && data.length == 0) {
        let message = `${DateTime.now().toFormat(customFullDateTimeFormat)} <strong>It's normally working at today.</strong>`
...
monitoringListItemModalTriggerList.forEach(element => {
    element.addEventListener('click', event => {
        ...
        monitoringListItemModalTitleEl.innerHTML = `${operatingSiteInfoColumn.toUpperCase()} Moniroting Chart`;
    ...
    .then(responseData => {
        ...
        let rows = data.map(element => {
            ...
            for (const key of Object.keys(element)) {
                if (element[key]['description_eng']) {
                    row[key] = element[key]['description_eng'];
...
function getPrimaryMonitoringLogLevelTypeCountChartSeriesData(data) {
    let seriesData = data.map(element => {
        ...
        switch (element['level'].toString()) {
            case essMonitoringLogLevel['warning']:
                logLevelDescription = 'Warning';

                break;
            case essMonitoringLogLevel['danger']:

                logLevelDescription = 'Fault';
                break;
            default:
                break;
        }
